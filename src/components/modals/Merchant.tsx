...
// Merchant account status states that block dashboard access on login.
// 'approved' and 'active' are treated as good-to-go statuses.
type BlockedStatus = 'pending' | 'suspended' | 'rejected'

const ALLOWED_LOGIN_STATUSES = ['approved', 'active']

// ─────────────────────────────────────────────────────────────────────────
// Component
// ─────────────────────────────────────────────────────────────────────────

export default function AuthModal({ isOpen, initialMode = 'register', onClose }: AuthModalProps) {
  ...
  // Set when a login succeeds but the merchant's account status isn't
  ...
  // Prefill referral code from ?ref= in the URL or from sessionStorage
  // (sessionStorage is set by app/signup/page.tsx when someone opens a
  // referral link, since that page redirects to "/" before this modal opens)
  useEffect(() => {
    if (!isOpen) return
    const stored =
    ...
    if (stored) {
      setForm((prev) => ({ ...prev, referralCode: stored.toUpperCase() }))
    }
  }, [isOpen])
  ...
  const resetAndClose = () => {
    onClose()
    // Give the close animation time to finish before wiping state
    setTimeout(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register' && !agreed) {
      setServerError('Please accept the Terms & Conditions and Merchant Agreement to continue.')
      return
    }
    if (!validate()) return
    ...
    try {
      if (mode === 'login') {
        ...
        // ── Check the merchant's account status before letting them in ──
        ...
        if (merchantError) {
          setServerError('Could not verify your account status. Please try again.')
          await supabase.auth.signOut()
          return
        }

        const status = merchantRow?.status?.toLowerCase() ?? null

        if (!status || !ALLOWED_LOGIN_STATUSES.includes(status)) {
          // Sign the session back out — we don't want an authenticated
          // session sitting around for an account that isn't cleared yet.
          await supabase.auth.signOut()

          if (status === 'pending' || status === 'suspended' || status === 'rejected') {
            setBlockedStatus(status as BlockedStatus)
          } else {
            // No merchant row found, or an unrecognized status value
            setServerError(
              'We could not find an active merchant account for this login. Please contact support.'
            )
          }
          return
        }
      ...
      if (error) {
        ...
        if (msg.includes('already registered')) {
          ...
          setServerError(
            'We could not create your account due to a server error. Please try again, or contact support if this continues.'
          )
        } else {
      ...
      // We have an active session (email confirmation is off) — insert the
      ...
      if (profileError) {
        if (profileError.code === '23505') {
          ...
          setServerError(profileError.message || 'Could not save your business details.')
        }
        return
      }
            ...
            <div className="max-h-[calc(90vh-6px)] overflow-y-auto px-6 pb-6 pt-6 sm:px-8 sm:pb-8">
              {!showResult ? (
                <>
                  ...
                  <div className="mb-5 pr-8">
                    <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#7BC142]/10 to-[#1857D6]/10 px-3 py-1 text-xs font-semibold text-[#3E7A1C]">
                      <Sparkles size={13} />
                      <span>Merchant Partner {mode === 'register' ? 'Registration' : 'Login'}</span>
                    </div>
                    <h2
                      id="merchant-modal-title"
                      className="text-2xl font-semibold text-[#0B0F19] sm:text-[28px]"
                    >
                      {mode === 'register' ? 'Become a Merchant Partner' : 'Welcome back'}
                    </h2>
                    <p className="mt-1.5 text-sm leading-relaxed text-slate-500">
                      {mode === 'register'
                        ? 'Register your business to get a dashboard, referral rewards, a shop QR code and exclusive merchant benefits.'
                  ...
                  <form onSubmit={handleSubmit} noValidate className="space-y-4">
                    <AnimatePresence mode="wait">
                      {mode === 'register' && (
                        ...
                        >
                          ...
                          {/* Owner Name */}
                          <Field label="Owner Name" icon={<User size={16} />} error={errors.ownerName}>
                            <input
                              type="text"
                    ...
                    {mode === 'register' && (
                      <label className="flex cursor-pointer items-start gap-2.5 pt-1 text-sm text-slate-600">
                        ...
                        <span>
                          ...
                          and{' '}
                          <a href="/merchant-agreement" className="font-medium text-[#1857D6] hover:underline">
                            Merchant Agreement
                          </a>
                          .
                        </span>
                    ...
                    >
                      ...
                      {submitting ? (
                        ...
                        <>
                          <span>
                            {mode === 'register' ? 'Register as Merchant Partner' : 'Sign In to Dashboard'}
                          </span>
                          <ArrowRight size={16} />
                        </>
                      )}
                ...
                <AccountStatusState
                  ...
                  onTryLoginAgain={() => setBlockedStatus(null)}
                />
              ) : (
                <SuccessState onDone={resetAndClose} businessName={form.businessName} />
              )}
            </div>
          </motion.div>
        </div>
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-sm text-sm leading-relaxed text-slate-500">
        Thanks{businessName ? `, ${businessName}` : ''} — your account is ready. Our team will
        review your details and approve your Merchant Partner account shortly. You&apos;ll
        receive your Merchant ID, referral code and shop QR code by email once approved.
      </p>
      <button
        onClick={onDone}
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-sm text-sm leading-relaxed text-slate-500">
        ...
        Click the link to activate your account, then log in to reach your Merchant Dashboard.
      </p>
      <button
        onClick={onDone}
        className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#1857D6] to-[#0B2E7A] px-6 py-3 text-sm font-semibold text-white shadow-[0_4px_16px_rgba(24,87,214,0.35)] transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(24,87,214,0.5)] cursor-pointer"
...
// Shown after a successful login when the merchant's account status is
// pending / suspended / rejected — the auth session has already been
// signed back out at this point, so this is purely informational.
function AccountStatusState({
  ...
  status: BlockedStatus
  onDone: () => void
  onTryLoginAgain: () => void
}) {
  const config: Record<
    BlockedStatus,
    { icon: React.ReactNode; iconWrap: string; iconColor: string; title: string; body: string }
  > = {
    pending: {
      icon: <Clock3 size={32} />,
      iconWrap: 'bg-gradient-to-br from-amber-400/15 to-amber-500/15',
      iconColor: 'text-amber-600',
      title: 'Your account is pending approval',
      body: 'Thanks for registering! Our team is still reviewing your business details. You\u2019ll be able to log in as soon as your account is approved — this usually doesn\u2019t take long.',
    },
    suspended: {
      icon: <ShieldAlert size={32} />,
      iconWrap: 'bg-gradient-to-br from-rose-400/15 to-rose-500/15',
      iconColor: 'text-rose-600',
      title: 'Your account has been suspended',
      body: 'Your Merchant Partner account is currently suspended and dashboard access is temporarily disabled. Please contact our support team to find out more or to resolve this.',
    },
    rejected: {
      icon: <XCircle size={32} />,
      iconWrap: 'bg-gradient-to-br from-slate-300/25 to-slate-400/25',
      iconColor: 'text-slate-600',
      title: 'Your registration was not approved',
      body: 'Unfortunately your Merchant Partner registration was not approved. If you think this is a mistake, please reach out to our support team for more details.',
    },
  }

  const { icon, iconWrap, iconColor, title, body } = config[status]
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center py-6 text-center"
    >
      <div className={`mb-4 flex h-16 w-16 items-center justify-center rounded-full ${iconWrap}`}>
        <span className={iconColor}>{icon}</span>
      </div>
      <h3 className="text-xl font-semibold text-[#0B0F19]">{title}</h3>
      ...
      <div className="mt-6 flex w-full flex-col gap-2.5 sm:flex-row sm:justify-center">
        {status === 'pending' && (
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-50 cursor-pointer"
          >
        ...
        <button
          onClick={onDone}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#1857D6] to-[#0B2E7A] px-6 py-3 text-sm font-semibold text-white shadow-[0_4px_16px_rgba(24,87,214,0.35)] transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(24,87,214,0.5)] cursor-pointer"
        >
          Close
        </button>
      </div>
