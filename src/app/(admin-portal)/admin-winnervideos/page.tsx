...
// ─────────────────────────────────────────────────────────────────────────
// Helpers: turn a raw URL into something embeddable in an <iframe>/<video>
// ─────────────────────────────────────────────────────────────────────────
...
export default function AdminWinnerVideosPage() {
  ...
  const [editingVideo, setEditingVideo] = useState<WinnerVideoRecord | null>(null)
  ...
  const BUCKET = 'winner_videos'
  const TABLE = 'winner_videos'

  // ── Fetch Winner Video Data ──────────────────────────────────────────
  const fetchVideos = async () => {
    ...
    try {
      ...
      setError('Failed to load winner video data.')
    } finally {
      setLoading(false)
    }
  }
  ...
  // ── Open Edit Modal ──────────────────────────────────────────────────
  const openEditModal = (video: WinnerVideoRecord) => {
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8" style={{ fontFamily: 'var(--font-display)' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Upload and manage winner announcement videos, highlight reels, and promotional links.
              </p>
            </div>
          </div>
          <button
            ...
            <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  <Trophy size={18} className="text-[#1857D6]" />
                  <h2 className="text-base font-semibold text-slate-900">Add New Winner Video</h2>
                </div>
              ...
              <form onSubmit={handleAddVideo} className="grid grid-cols-1 gap-4 sm:grid-cols-12">
                <div className="sm:col-span-4">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g., Diwali Contest Winner Announcement"
                    value={newTitle}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-8">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="p-2 bg-blue-50 rounded-lg"><Film size={16} className="text-[#1857D6]" /></div>
          </div>
          <h3 className="text-2xl font-bold text-slate-900">{totalVideos}</h3>
          <p className="text-xs text-slate-400 mt-1">Winner videos published</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          <p className="text-xs text-slate-400 mt-1">Active on YouTube</p>
        </div>
      ...
      >
        <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div>
            <h2 className="text-base font-semibold text-slate-900">Uploaded Winner Videos</h2>
              ...
              {filteredVideos.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-slate-400">
                    No winner videos found matching your criteria.
                  </td>
                </tr>
              ) : (
                filteredVideos.map((v) => (
                  <tr key={v.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    {/* Small clickable preview box */}
                    <td className="py-4 px-4">
                      <button
                        onClick={() => setPreviewVideo(v)}
                        title="Play video"
                        className="group relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg bg-slate-900 shadow-sm transition-transform hover:scale-105"
                      >
                        <div className="absolute inset-0 bg-gradient-to-br from-[#1857D6]/40 to-slate-900" />
                        <Play size={16} className="relative z-10 fill-white text-white" />
                      </button>
                    </td>
        ...
        {previewVideo && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl rounded-3xl border border-slate-200/80 bg-white p-4 shadow-2xl sm:p-5"
            >
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <h3 className="truncate text-sm font-semibold text-slate-900">{previewVideo.title}</h3>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <a
                    ...
                    className="rounded-full p-1.5 text-slate-400 hover:bg-slate-100 hover:text-[#1857D6]"
                  >
            ...
            >
              <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-semibold text-slate-900">Edit Winner Video</h3>
                </div>
                <button
                  onClick={() => setEditingVideo(null)}
