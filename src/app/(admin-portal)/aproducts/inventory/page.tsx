...
export default function InventoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Using select('*') ensures the query doesn't fail if a specific column is missing
  ...
  const handleUpdate = async (id: string, value: string) => {
    ...
    const { error } = await supabase.from('products').update({ stock: numVal }).eq('id', id)
    if (!error) {
      setProducts(prev => prev.map(p => p.id === id ? { ...p, stock: numVal } : p))
    } else {
  ...
  const processedProducts = useMemo(() => {
    let result = products.filter(p => {
      ...
      if (filterStock === 'In Stock') matchesStock = stockVal > 5
      if (filterStock === 'Low Stock') matchesStock = stockVal > 0 && stockVal <= 5
      if (filterStock === 'Out of Stock') matchesStock = stockVal === 0
    ...
    result.sort((a, b) => {
      ...
      if (sortOrder === 'stock_high') return stockB - stockA
      if (sortOrder === 'stock_low') return stockA - stockB
    ...
    return result
  }, [products, searchQuery, filterCategory, filterSubCategory, filterStock, sortOrder])
    ...
    <div className="mx-auto max-w-8xl bg-white px-4 py-8 sm:px-6 lg:px-8" style={{ fontFamily: 'var(--font-display)' }}>
      ...
      <div className="relative mb-6 overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="relative z-10 flex items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-tr from-[#1857D6] to-[#0B2E7A] text-white shadow-lg shadow-blue-500/20"><Boxes size={30} /></div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Inventory Management</h1>
            <p className="mt-1 text-sm text-slate-500">Track and update your product stock quantities.</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-100">
                ...
                <th className="py-3 px-4 font-semibold uppercase tracking-wider text-xs">Current Stock</th>
                  ...
                  <td className="py-4 px-4">
                    {editingId === p.id ? (
                      <input 
                        ...
                        id={`inv-${p.id}`} 
                        className="w-24 rounded-lg border border-slate-300 bg-white text-slate-900 px-3 py-1.5 text-sm font-semibold shadow-sm focus:outline-none focus:ring-2 focus:border-[#1857D6] focus:ring-[#1857D6]/20" 
                      />
                    ) : (
                      <div className="flex flex-col">
                        <span className="font-bold text-slate-900">{p.stock || 0} units</span>
                        {p.stock === 0 ? (
                          <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider mt-0.5">Out of Stock</span>
                        ) : p.stock <= 5 ? (
                          <span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider mt-0.5">Low Stock</span>
                        ) : null}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-end gap-2">
                      {editingId === p.id ? (
                        <>
                          <button 
                            onClick={() => handleUpdate(p.id, (document.getElementById(`inv-${p.id}`) as HTMLInputElement).value)} 
