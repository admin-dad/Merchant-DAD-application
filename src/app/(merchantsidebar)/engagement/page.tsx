...
interface ScanRecord {
  id: string
  customer_name: string | null // Added customer_name
...
export default function CustomerEngagementPage() {
  ...
  useEffect(() => {
    const fetchEngagementData = async () => {
      ...
      // Fetch all scans including customer_name
  ...
  // New vs Returning (based on lifetime)
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8" style={{ fontFamily: 'var(--font-display)' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Monitor walk-in customer scans, track returning visitors, and analyze shop traffic.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        {/* Lifetime Stats & Recent Activity (Left Column - 7/12) */}
        <motion.div 
        ...
        >
          <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-900">Recent Customer Activity</h2>
              <p className="text-xs text-slate-500">Live feed of the latest QR scans at your shop.</p>
            </div>
          </div>
              ...
              scans.slice(0, 15).map((scan) => (
                <div key={scan.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-200/80 hover:border-slate-300 hover:bg-slate-50/50 transition-all duration-200">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      {/* Displays customer name if present, otherwise phone number or walk-in */}
                      <p className="text-sm font-semibold text-slate-900">
                        {scan.customer_name ? scan.customer_name : scan.customer_phone ? `+91 ${scan.customer_phone}` : 'Walk-in Customer'}
                      </p>
                      <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                        <Clock size={12} />
                        {formatDate(scan.created_at)}
                        {scan.customer_name && scan.customer_phone && ` • +91 ${scan.customer_phone}`}
                      </p>
                    </div>
                  </div>
        ...
        {/* Lifetime Summary (Right Column - 5/12) */}
        <motion.div 
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3, delay: 0.1 }}
